            ...
            if rec.workorder_ids:
                for record in rec.workorder_ids:
                    if record.state == 'progress':
                        rec.workorder_ids_state = record.name
                        break

                    else:
                        rec.workorder_ids_state = ""
            else:
                rec.workorder_ids_state = ""
    ...
    def _compute_quality_widget(self):
        for production in self:
            ...
            other_check = production.check_ids.filtered(
                lambda check: check.id not in production.workorder_ids.mapped('check_ids').ids and check.point_id)
            if other_check:
