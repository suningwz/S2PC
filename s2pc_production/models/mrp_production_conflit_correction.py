...
class MrpProduction(models.Model):
    ...
    date_planned_start_related = fields.Datetime(string="Infos date prevue", related="date_planned_start",
                                                 readonly=True)
    workorder_ids_statee = fields.Char(string="Opération en cours", compute="get_state_workoderids")
    ...
    def get_state_workoderids(self):
        for rec in self:
            if rec.workorder_ids:
                for record in rec.workorder_ids:
                    if record.state == 'progress':
                        rec.workorder_ids_statee = record.name
                        break

                    else:
                        rec.workorder_ids_statee = ""
            else:
                rec.workorder_ids_statee = ""
    ...
    def _compute_quality_widget(self):
        for production in self:
            ...
            quality_vals['header_check'] = ['Opération', 'Poste de travail', 'Contrôlé par', 'Point de contrôle',
                                            'Etat', 'Notes']
            quality_vals['header_alert'] = ['Opération', 'Nom', 'Poste de travail', 'Responsable', 'Cause première',
                                            'Etape', 'Description']
            for work in production.workorder_ids:
                if work.check_ids:
                    ...
                    for check in work.check_ids:
                        ...
                        check_vals.update(
                            measure_on=self.get_selection_label('quality.check', 'measure_on', check.measure_on))
                        ...
                        check_vals.update(quality_state=self.get_selection_label('quality.check', 'quality_state',
                                                                                 check.quality_state))
            ...
            other_check = production.check_ids.filtered(
                lambda check: check.id not in production.workorder_ids.mapped('check_ids').ids)
            if other_check:
                ...
                for check in other_check:
                    ...
                    check_vals.update(additional_note=check.additional_note)
                    check_vals.update(
                        quality_state=self.get_selection_label('quality.check', 'quality_state', check.quality_state))
        ...
        for production in self:
            for workorder in production.workorder_ids:
                moves = production.move_raw_ids.filtered(
                    lambda move: move.bom_line_id.restricted_operation_id == workorder.operation_id)
                for move in moves:
    ...
    def get_bom(self):
        if self.workorder_ids:
            print("misy valeur ah")
        else:
            print("tsy misy valeur ah")
